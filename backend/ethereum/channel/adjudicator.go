...
import (
	...
	wallet2 "perun.network/go-perun/backend/ethereum/wallet"
...
func toEthSignedStates(subChannels []channel.SignedState) (ethSubChannels []adjudicator.AdjudicatorSignedState) {
	...
	for i, x := range subChannels {
		ethSubChannels[i] = adjudicator.AdjudicatorSignedState{
			...
			Sigs:   signatureToByteArrays(x.Sigs),
		}
	}
	return
}

func signatureToByteArrays(signs []wallet.Sig) [][]byte {
...
func byteArraysToSignature(byteArrays [][]byte) []wallet.Sig {
	sigs := make([]wallet.Sig, len(byteArrays))
	for i := range byteArrays {
		sig := (wallet2.Sig)(byteArrays[i])
		sigs[i] = &sig
	}
	return sigs
}
...
func (a *Adjudicator) call(ctx context.Context, req channel.AdjudicatorReq, fn adjFunc, txType OnChainTxType) error {
	...
	tx, err := func() (*types.Transaction, error) {
		...
		tx, err := fn(trans, ethParams, ethState, signatureToByteArrays(req.Tx.Sigs))
		if err != nil {
