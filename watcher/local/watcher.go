...
type (
	...
	ch struct {
		...
		parent         channel.ID
		children       map[channel.ID]struct{}
		closedChildren map[channel.ID]channel.SignedState
...
func (w *Watcher) StartWatchingSubChannel(ctx context.Context, parent channel.ID, signedState channel.SignedState) (
	...
	parentCh.children[signedState.State.ID] = struct{}{}
...
func newCh(id, parent channel.ID, params *channel.Params,
	...
	return &ch{
		...
		parent:         parent,
		children:       make(map[channel.ID]struct{}),
		closedChildren: make(map[channel.ID]channel.SignedState),
...
func retreiveLatestSubStates(r *registry, parent *ch) (channel.Transaction, []channel.SignedState) {
	...
	for i := range parentTx.Allocation.Locked {
		...
		if ok {
			...
			subStates[i] = parent.closedChildren[parentTx.Allocation.Locked[i].ID]
		}
	}
...
func (w *Watcher) StopWatching(id channel.ID) error {
	...
	if parent != channel.Zero { // Sub channel.
		...
		if _, ok := parentCh.retreiveLatestTx().SubAlloc(id); ok {
			parentCh.closedChildren[id] = makeSignedState(ch.params, ch.retreiveLatestTx())
		}
		delete(parentCh.children, id)
	} else { // Ledger channel.
		ch.subChsAccess.Lock()
		defer ch.subChsAccess.Unlock()

		if len(ch.children) > 0 {
			return fmt.Errorf("cannot de-register when sub-channels are present: %d %v", len(ch.children), ch.id)
		}
	}
