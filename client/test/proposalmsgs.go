...
func ProposalMsgsSerializationTest(t *testing.T, serializerTest func(t *testing.T, msg wire.Msg)) {
	...
	subChannelProposalSerializationTest(t, serializerTest)
}

func channelProposalReqSerializationTest(
	t *testing.T,
...
func channelProposalAccSerializationTest(
	t *testing.T,
	serializerTest func(t *testing.T, msg wire.Msg),
) {
	t.Helper()
...
func channelProposalRejSerializationTest(
	t *testing.T,
	serializerTest func(t *testing.T, msg wire.Msg),
) {
	t.Helper()
...
func subChannelProposalSerializationTest(
	t *testing.T,
	serializerTest func(t *testing.T, msg wire.Msg),
) {
	t.Helper()
	...
	for i := 0; i < repeatRandomizedTest; i++ {
		m, err := NewRandomSubChannelProposal(rng)
		require.NoError(t, err)
		serializerTest(t, m)
	}
}
...
func newRandomString(rng *rand.Rand, minLen, maxLenDiff int) string {
	...
	rng.Read(r)
	return toValidASCIIString(r)
}

func toValidASCIIString(raw []byte) string {
	for i := range raw {
		raw[i] &= 0 // valid ascii range is 0-127.
	}
	return string(raw)
